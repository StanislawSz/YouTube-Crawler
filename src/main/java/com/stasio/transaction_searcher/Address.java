...
public class Address {
    ...
    public Address (String hash160, String address, long totalReceived, long totalSent, long finalBalance, int txCount, List<SingleTransaction> transactions) {
    ...
    public Address (JsonObject a) {
        ...
        transactions = new ArrayList<SingleTransaction>();
        for (JsonElement txElem : a.get("txs").getAsJsonArray()) {
            JsonObject addrObj = txElem.getAsJsonObject();
            transactions.add(new SingleTransaction(addrObj));
        }
    }
